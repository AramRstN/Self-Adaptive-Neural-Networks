...
class FakeArgs:
    def __init__(self, parser):
        ...
        self.params = {
            ...
            'prune': True,
            'method': 'l1',
            'soft-keeping-ratio': 0.5,
            'pretrained': True,
            'output-dir': 'run/imagenet/mobilenet_v2_gnorm',
            'target-flops': 2.04, 
            'global-pruning': True,
            'print-freq': 100,
            'workers': 8,
            'cache-dataset': False
            #'data-path' : '~/train_imagenet/data/imagenet/'
        }
        self.params = {k.replace('-', '_'): v for k, v in self.params.items()}
    def __getattr__(self, name):
        if name in ['parser', 'params']:
...
def patch_arg_parser(arg_parser_get):
    def patched(*args, **kwargs):
        parser = arg_parser_get(*args, **kwargs)
        return FakeArgs(parser)
    return patched
